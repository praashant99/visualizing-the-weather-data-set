...
def line_chart(df,period,col):
    f=df.reset_index().plot.line(x=period,y=col)
    plt.show()
    return f 

line=line_chart(weather_df,'Date/Time','Temp (C)')
plt.show()
...
def plot_categorical_columns(df):
    f=df.plot.bar(x='Temp (C)',y='Visibility (km)')
    return f 
cat=plot_categorical_columns(weather_df)
plt.show()
...
def plot_cont(df,plt_typ):
    f=df.boxplot(column='Rel Hum (%)',by='Wind Spd (km/h)')
    return f 

pltt=plot_cont(weather_df,'boxplot')
plt.show()
...
def group_values(df,col1,agg1,col2):
    f=df.groupby([col1])[col2].agg(agg1).plot(kind='bar')
    return f

meann=group_values(weather_df,'Weather',np.mean,'Visibility (km)')
plt.show()
